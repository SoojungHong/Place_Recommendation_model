...
def plotOneCluster(X_embedded, x_val, num_datapoints):
    plt.figure(figsize=(15,15))
    for i in range(num_datapoints): #number of data points in plot (number of vectors)
...
def plotOneClusterWithXandY(X_embedded, x_val, y_val, num_datapoints):
    plt.figure(figsize=(15,15))
    for i in range(num_datapoints): #number of data points in plot (number of vectors)
        if (X_embedded[i,0] < x_val) and (X_embedded[i, 1] < y_val): #-55): 
            plt.scatter(X_embedded[i, 0], X_embedded[i, 1], c="r", cmap=plt.cm.get_cmap("Dark2", 7)) 
        else: 
            plt.scatter(X_embedded[i, 0], X_embedded[i, 1], c="g", cmap=plt.cm.get_cmap("Dark2", 7)) 
...
def plotOneClusterWithRange(X_embedded, x_val_l, x_val_r, y_val_d, y_val_u, num_datapoints):
    plt.figure(figsize=(15,15))
    for i in range(num_datapoints): #number of data points in plot (number of vectors)
        if (X_embedded[i,0] > x_val_l) and (X_embedded[i,0] < x_val_r) and (X_embedded[i, 1] > y_val_d) and (X_embedded[i, 1] < y_val_u): 
            plt.scatter(X_embedded[i, 0], X_embedded[i, 1], c="r", cmap=plt.cm.get_cmap("Dark2", 7)) 
        else: 
            plt.scatter(X_embedded[i, 0], X_embedded[i, 1], c="g", cmap=plt.cm.get_cmap("Dark2", 7)) 
...
def getItemsInCluster(X_embedded, x_val, num_datapoints):
    item_index_in_cluster = []
    
    for i in range(num_datapoints): #number of data points in plot (number of vectors)
        if (X_embedded[i,0] < x_val): 
            item_index_in_cluster.append(i)
    return item_index_in_cluster



def getItemsInClusterWithXandY(X_embedded, x_val, y_val, num_datapoints):
    item_index_in_cluster = []
    
    for i in range(num_datapoints): #number of data points in plot (number of vectors)
        if (X_embedded[i,0] < x_val) and (X_embedded[i,1] < y_val): 
            item_index_in_cluster.append(i)
    return item_index_in_cluster


def getItemsInClusterWithRange(X_embedded, x_val_l, x_val_r, y_val_d, y_val_u, num_datapoints):
    item_index_in_cluster = []
...
#-----------------------------------------------------------------------------
# example to select the cluster and show the place names of selected cluster
num_datapoints = X_embedded.shape[0]
x_val = -55
plotOneCluster(X_embedded, x_val, num_datapoints)
items_indices = getItemsInCluster(X_embedded, x_val, num_datapoints)
# with items indices, get place names of these items 
item_factorized_index_in_dataset = []
for k in range(len(items_indices)): 
    idx = items_indices[k]
    item_factorized_index_in_dataset.append(item_index_from_org_dataset[idx])

placeNamesInCluster = getItemsPlaceName(item_factorized_index_in_dataset, items_indices, item_index_from_org_dataset)
placeNamesInCluster

#--------------------------------------
# give x and y condition in plot 
num_datapoints = X_embedded.shape[0]
#x_val = -20
#y_val = -40
x_val_l = 2.5
...
y_val_d = 1.5
y_val_u = 5
#plotOneClusterWithXandY(X_embedded, x_val, y_val, num_datapoints)
plotOneClusterWithRange(X_embedded, x_val_l, x_val_r, y_val_d, y_val_u, num_datapoints)
test_items_indices = getItemsInClusterWithRange(X_embedded,  x_val_l, x_val_r, y_val_d, y_val_u, num_datapoints)
len(test_items_indices)
test_item_factorized_index_in_dataset = []
for k in range(len(test_items_indices)): 
    idx = test_items_indices[k]
    test_item_factorized_index_in_dataset.append(item_index_from_org_dataset[idx])

test = getItemsPlaceName(test_item_factorized_index_in_dataset, test_items_indices, item_index_from_org_dataset)
test
...
gmap2 = gmplot.GoogleMapPlotter.from_geocode(center_lat, center_lon, 18)
gmap2.draw("C:/Users/shong/Documents/data/place_oct30.html")
